 ...
 * @author danyelf
 */
public class AddNodeStohDemo2 extends javax.swing.JApplet {
...
//            100);
	...
	public void init() {
		...
		double[] probEdgesToAttachMonad = new double[] { 0., 0, 1 };
		double[] probEdgesToAttachDiad = new double[] { 0., 0, 1 };

		double gam=1;
		double p=1;
		genBA = new GenNonIntBA5(vertexFactory, edgeFactory, probEdgesToAttachMonad, probEdgesToAttachDiad,
				paBA, gam, p);
...
//        ((FRLayout)layout).setMaxIterations(200);
		...
		vv.getRenderContext().setVertexFillPaintTransformer(new Transformer<Number, Paint>() {
			...
			public Paint transform(Number arg0) {
				...
				if (i < 0)
					return Color.BLACK;
				System.out.println("i="+i+" num="+g.getVertexCount());
				if (i > g.getVertexCount()-6) 
					return Color.RED;
	...
	public void process() {
		...
		try {
			System.out.println("counts:"+g.getVertexCount());
	...
	public static void main(String[] args) {
		AddNodeStohDemo2 and = new AddNodeStohDemo2();
		JFrame frame = new JFrame("Complex increment");
